...
int luckBalance(int k, vector<vector<int>> contests) {
    vector<int> imp_cont;
    int lucky = 0;
    for(uint i = 0; i < contests.size(); i++){
        if(contests[i][1] == 0){
            lucky += contests[i][0];
        }
        else{
            imp_cont.push_back(contests[i][0]);
        }
    }
    
    sort(imp_cont.begin(), imp_cont.end(), greater<int>());
    for (int i = 0; i < imp_cont.size(); ++i) {
        if (i < k) lucky += imp_cont[i];
        else lucky -= imp_cont[i];
    }
    return lucky;
}
