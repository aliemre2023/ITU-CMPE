...
public:
    ...
    QuadruplySkipList(int height_in) : height(height_in) {
        ...
        for (int i = 0; i < height_in-1; i++) {
            ...
            ptr->next->below = ptr->below->next;
            ptr->below->above = ptr;
            ptr->below->next->above = ptr->next;
            ptr->below->next->prev = ptr->below; 
            ptr = ptr->below;
        }
    }
...
void QuadruplySkipList::insert(Employee* emp) {
    ...
    while(ptr){
        ...
        if (above_temp != nullptr) {
            ...
            //above_temp->below->above = above_temp->below;
        }
        above_temp = new_emp;
        new_emp->prev = ptr;
        ...
        QuadruplySkipList_Node* temp_next = ptr->next; 
        new_emp->next = temp_next;
        ptr->next = new_emp;
        ptr = ptr->below;
    }
}
...
void QuadruplySkipList::remove(int id) {
    ...
    if (ptr->next && ptr->next->data->get_id() == id) {
        ...
        //delete temp->data; // cant work properly 
        //delete temp;
        temp->data = nullptr;
        temp = nullptr;
    }
